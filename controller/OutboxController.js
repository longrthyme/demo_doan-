...
class OutboxController {
    ...
    async read(req, res) {
        ...
        if (!userId) {
            return res.status(200).redirect('/'); // No userId means redirect to home
        }
        ...
        try {
            ...
            if (rows.length > 0) {
                return res.status(200).render('OutboxPage', { data: rows, page, iterator, endingLink, numberOfPages, USERNAME, EMAIL });
            }
